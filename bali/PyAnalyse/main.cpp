...
int AnalyseOfCpp(byte user_tele_ori[],char* AnalyseResult);

extern "C"
{
  int Analyse(byte user_tele_ori[],char *AnalyseResult)
  {
    AnalyseOfCpp(user_tele_ori,AnalyseResult);
  }
}
...
//  cout<<user_tele<<endl;
  sResult += "\n对以下用户报文进行解析:\n";
  sResult += (char *)user_tele;

//  cout<<"解析结果:"<<endl;

  sResult += "解析结果:\n";
  Analyse_Usertele_Info(user_tele,len_usertele,sResult);

  for(i=0;i<strlen(sResult.c_str());i++)
	AnalyseResult[i] = sResult[i];
  AnalyseResult[i] = '\0';
