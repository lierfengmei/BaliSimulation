...
# first use analyse to show the results
def analyse():
...
#    output2 = create_string_buffer(b'\0'*1000)
   ...
   # obj.Analyse(user_tele,byref(output),byref(output2))
    
    obj.Analyse(user_tele,byref(output))

    print(output.value.decode('utf-8'))
   # print(output2.value.decode('utf-8'))


def Analyse(tele_ori):
    print("tele_ori is")
    print(tele_ori)
